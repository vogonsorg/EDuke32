...
static void Menu_PopulateVideoSetup()
{
    ...
    {
        MEOSN_VIDEOSETUP_DISPLAY[i] = videoGetDisplayName(i);
...
static void msaveloadtext(const vec2_t& origin, int level, int volume, int skill, const char *boardfn, int16_t health)
{
    ...
    //lcheck(localeLookup(vname));
    //lcheck((savehead.volnum == 0 && savehead.levnum == 7) ? boardfn : name ? localeLookup(name) : nullptr);
    //lcheck(localeLookup(g_skillNames[skill-1]));
